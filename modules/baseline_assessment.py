...
class BaselineAssessment:
    ...
    def _analyze_energy_consumption(self):
        ...
        if 'energy_consumption' in self.datasets:
            ...
            if 'production' in self.datasets:
                ...
                if 'timestamp' in energy_df.columns and 'timestamp' in prod_df.columns:
                    ...
                    # Group by date before merging
                    energy_daily = energy_df.groupby('date')['kwh_consumed'].sum().reset_index()
                    prod_daily = prod_df.groupby('date')['production_tons'].sum().reset_index()
                    
                    # Merge and calculate intensity
                    merged = pd.merge(energy_daily, prod_daily, on='date', how='inner')
    ...
    def _analyze_material_flow(self):
        ...
        if 'material_input' in self.datasets and 'material_output' in self.datasets:
            ...
            # Calculate material cost if available
            if 'cost_per_ton' in input_df.columns:
                input_df['total_cost'] = input_df['quantity_tons'] * input_df['cost_per_ton']
                results['total_material_cost'] = input_df['total_cost'].sum()
                results['specific_material_cost'] = results['total_material_cost'] / results['total_input']
    ...
    def _generate_energy_charts(self, output_dir: str):
        ...
        if 'consumption_by_area' in energy_results:
            ...
            # Create a bar chart
            plt.bar(areas, values, color='#FF9800')
        ...
        if 'energy_intensity_trend' in energy_results:
            ...
            plt.plot(dates, intensities, marker='o', color='#FF9800')
    ...
    def _generate_material_flow_diagrams(self, output_dir: str):
        ...
        if 'input_by_type' in material_results:
            ...
            plt.bar(materials, quantities, color='#FF9800')
